...
│   ├── abilities2.0/     #   AbilityData — enemy abilities (§7.6): name, type,
│   │                     #   argument shape, sentence, and the `Effect` DSL
│   │                     #   (`trigger: op args`) GameLoop2 dispatches on
...
| `LootSystem` | The one place a piece of carried loot is SPENT. Each kind owns its own resolution; what they share is everything around a use — consuming the entry, **Echo Chamber**'s copies of the last three used, and the memory they read (`GameState.loot_used_memory`). It belongs to neither system because an echo of a pill can be a scroll. Isaac's ordering: the copies fire off the memory as it stood *before* this use, so nothing echoes itself and no echo is remembered. **A wand zap is copied like any other use** — the charge is spent once, by `use_loot`, and a copy only re-runs the zap (docs/loot-passives.md §9). **A passive piece is never spent at all** — a trinket, or a card whose line opens "Passive:" — and `use_loot` refuses one. What those pieces do from their slot is `LootPassives` (`scripts/runtime/`, docs/loot-passives.md): it hands each working piece to the relic runner as a relic-shaped source, resolves Blueprint's "the slot to its right" through the pack's arrangement, and is why moving a piece can change what the pack does. |
| `GameLog` | Verbose run-scope message log (games taken, enemies that walked on, events, items, loot, shop purchases, lost runs) — the written record behind the toasts. Each line is stamped with the game the run was standing on when it was written, which is what lets **`RunLogScreen`** (the header's `🕮 History` button) group a run into its stops. |
| `Notifications` | Curated player-facing "important events" channel; the overworld mounts `NotificationToasts` to show them. A notice can carry a picture and a source key: every relic or pack passive that fires posts one with its art, and repeats from one source stack into one toast (`×N`) — docs/loot-passives.md §5. |
| `SaveSystem` | Save/load for a games-first run (`user://`): a named save per run plus the run's own autosave slot. Writes GameState, `GameLoop2`, and the overworld's on-screen state, and hands a loaded run back to the next `Overworld2` to boot. Written **beside and renamed over**, never straight onto the live file: `FileAccess.WRITE` truncates before the first byte lands, and the autosave is rewritten every time the run's position changes, so the old shape opened that window dozens of times a run on the one file the player never chose to keep. The payload is built first, written to `<path>.tmp`, and moved into place — the save on disk is wholly the old run or wholly the new one. |
  ...
  **Exit Game is a MAIN MENU button and only works on the main menu.** It is
  ...
  - **`CustomRunScreen.gd`** — **⚙ Custom Run**: build a run out of a chosen set of
    ...
    **the amulet** — each with the same four axes (library, genre, record, release
    ...
    length (how many games from start to Amulet), an optional **seed**, and an
    optional **named target**, which composes with the band rather than overriding
    it. Begin hands the configuration to `RunConfig` and then opens the ordinary
    character picker, so a custom run is the normal flow with a screen in front of
    it.
    The **seed** is the run written down: type a number and the same run comes
    back — the same Amulet, the same opening three with the same bodies on them,
    the same drops, level-up rolls and capsule alphabet. Left blank the run rolls
    its own, which is what every run did before the box existed. It is a text
    field rather than a spinner because a seed is something you paste in off
    someone else's screenshot, and it is capped at 18 digits so what is on screen
...
- **`Overworld2.gd`** — the run itself: the offering of games
  ...
  and wear a ROUND box rather than a square one, and they ARM rather than resolve — the box goes on and off freely, there is no
  ...
  **`🗺 Map` and `→ Optimal Path` are two buttons, two destinations and two
  ...
  raised the whole 882-star **Atlas** — and the *ladder*, which is what actually
...
the ratio. Worst text on the page: **4.81:1** against an AA bar of 4.5. It also
...
figures below are the real page measured at **352 wide** (the heavy column is the
...
**The row itself is a 32px picture and its text, and nothing else.** The `□`/`✓`
checkbox each row used to open with is gone: it spent 21px of every row (the box
and its gutter) saying what the row already says twice — a finished goal is
struck through, dimmed, and its art drained to greyscale, and the row flashes
green at the moment it is crossed off. The art moved into that space and grew
from 24px to 32px, so the list opens on a column of faces. That is where the
28px this column lost came from: the checklist has more room for text than it
had at 380, not less.
...
current game over the destination. At 352 each half has ~141px for its title,
which is still two words; take another 40 away and it is one word and an
ellipsis on both sides, which is worse than the stacked row the columns replaced.
**352 rather than a rounder 350 is deliberate** — 350 sits *on* that breakpoint,
and a default one rounding error away from rendering the other layout is a
default nobody has tested.
...
**586 + 91 is where the 680 above comes from**: at that height a burst of toasts
...
`--goal-art` (32px). Shrinking the card is the right lever here; scaling the OBS
scene item resamples the pixel art instead.

**To rename the chat command**, edit the `.lookup` line in `obs/overlay.html` —
it is plain text in the page, not something the game sends, because the game has
no idea what anyone's bot is called. If you would rather not touch the installed
file (it is overwritten from `res://obs/` at every boot), hide it from
`user://obs/custom.css` with `.lookup { display: none }` and put the command in
an OBS text source instead. Whatever it says, keep it to **one line**: it is the
only text on this page a viewer has to read character by character and type, and
`check_overlay.js` fails if it wraps or clips.
...
between the overlay's halves, and the game keeps 81% of the width.
...
| Camera | `0, 0` | `384 × 216` |
| Chat | `0, 224` | `384 × 856` |
| Game capture | `392, 200` | `1152 × 648` (16:9) |
| **Overlay** | `1552, 0` | `352 × 680` |

The game drops to 60% of the width and the middle column carries ~430px of dead
...
`1552, 16` at `352 × 680`. The game keeps every pixel and the columns cover ~20%
...
crossed off. It is headed by **`!roguelikelike for challenge details`** and then
**THE GOALS** — the chat command first, because it is the answer to the question
...
| `_abilities_effect_column_setup.py` | one-shot: filled the `abilities` sheet's `Effect` column for all 31 rows — the `trigger: op args` DSL `GameLoop2` now dispatches on. Before it, the column was empty in every row and each ability was a hardcoded branch keyed by id, which made abilities the one content type whose behaviour was not authored upstream (§7.6) |
...
| `check_data_sync.py` | **is `data/` still what the sheet says?** Runs every generator over the checked-in workbook and asks git whether anything moved, then puts the tree back. This is the rule in CLAUDE.md ("the spreadsheet is upstream of `data/`") with something behind it: the first run found three rows that had drifted, two of them in one direction and one in the other. `check_map_sync.py` covers `data/games/`; this covers the other twenty folders. Run by CI. |
| `_xlsx_surgery.py` | shared helper: edit ONE sheet of `Roguelikes.xlsx` in place. An openpyxl round-trip drops the workbook's eight charts, so the sheet-editing one-shots (`_statuses_sheet_setup.py`, `_statuses2_combat_setup.py`, `_statuses2_burn_setup.py`, `_items2_statuses_setup.py`, `_events2_sheet_setup.py`, `_curses2_sheet_setup.py`, `_abilities_sheet_setup.py`, `_abilities_effect_column_setup.py`) rewrite just that sheet's XML parts and copy every other zip entry through untouched. |
...
- **Content depth** — the catalogs are thin next to the 890 games: 48 goal-enemies,
