...
| `Ownership` | Which real games the player owns, and where that answer comes from: the catalog's shipped `GameData.owned` column, or **the player's own list** — seeded by syncing a public Steam profile and edited by ticking the mark on a game's cover in the Collection. `Ownership.is_owned(game)` is the single read; every owned filter, the atlas's owned rings and the settings counts go through it, and the `.tres` column is never written to. Per-profile, in the active profile's `ownership.cfg`. |
