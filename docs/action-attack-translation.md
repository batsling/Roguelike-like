...
| `arc=<deg>` | explicit swing arc in degrees — overrides the size-word arc | `swing` |
...
For `swing` the size word **also sets the arc** — the action mirror of the
strategy grid's swing tiles: **Small** wraps just the front (110°, the grid's
3 front tiles), **Medium** wraps onto the flanks (200°, front 3 + 2 sides),
**Large** is the full all-around ring (360°, all 8 neighbours). `Swing, Large`
replaces the old `Swing, arc=360` spelling; an explicit `arc=` still overrides.
Arc tunables live on `ActionAttackLibrary.swing_arc_by_size`.
...
`poke` stays a thrust and a Large swing (360°) stays a full ring. Tunables
live on `ActionAttackLibrary` (`swing_duration`, `swing_trail_segments`).
...
| `swing` | arc cone at melee reach; the size word sets the arc (Small 110° / Medium 200° / Large 360°) | white arc smear (Large = instant ring) | size=Medium |
