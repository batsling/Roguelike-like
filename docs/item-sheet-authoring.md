...
**`bank_shields` is gone, and so are Barricade and Ride the Bus.** Both were
relics tagged `card` on this sheet, and both are **cards** now
([`cards-design.md`](cards-design.md) §5.1) — their rows are deleted from `items`
and their `.tres` from `data/items2.0/`. Barricade changed meaning in the move: the
relic banked every resolved game's leftover Temporary Shields forever, and the card
banks the next game's and is then spent. `bank_shields` left this vocabulary with
it, because Barricade was its only author and a keyword nothing can write is a
keyword that rots.
...
- **Mine-r Construction**'s `Reference` → `BroomSweeper`, which is how the
  `games` sheet spells it (it arrived as "Broomsweeper").
