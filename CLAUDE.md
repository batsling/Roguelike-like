...
| what the layout pass left open | `docs/layout-review-backlog.md` — all eight items closed. Fonts and gaps are both on `UITheme`'s scales project-wide, with `test_design_tokens.gd` asserting both lists complete; what remains is optional — 57 gaps typed BETWEEN two steps (`OFF_SCALE_GAPS`), which can be snapped one screen at a time by looking at the running screen. All of it stays in the doc with its reasoning so none of it gets re-litigated. Two standing notes in it are worth reading before touching any UI: judge colour by sampling the rendered pixel rather than by eye off a screenshot (that produced one confidently wrong finding), and use the `verify` skill to look at a screen rather than reasoning about it |
...
- **The varying failures that used to live here are FIXED.** All were
  ...
  - `test_overworld2.gd::test_leaving_the_haul_screen_lands_the_shelf_under_the_board`
    ...
    offering, and once in a while that opening game was one of the ten hubs
    (which sold until §19.7) — at which point walking off its haul screen
    mounted that hub's shelf, exactly as it should. The test then asserted `_shop_panel` was null "while the haul is
...
- **Game covers load lazily.** `GameData.cover_path` holds the path and
  ...
  back into an `@export var ... : Texture2D` — `Data` loads all 882 games at
...
- **`data/games/` has 882 files.** Never glob or read it wholesale to answer a
