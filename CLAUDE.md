...
| what the layout pass left open | `docs/layout-review-backlog.md` — eight items, six still open, with what, why and what it would take. §1 (tier-list palette) is decided and §2's font half is done; both stay in the doc with their reasoning so neither gets re-litigated. Two standing notes in it are worth reading before touching any UI: judge colour by sampling the rendered pixel rather than by eye off a screenshot (that produced one confidently wrong finding), and use the `verify` skill to look at a screen rather than reasoning about it |
