...
static func make_health_potion(pos: Vector2i) -> Item:
	var it = Item.new()
	it.grid_pos = pos
	it.glyph = "!"
...
static func make_strength_scroll(pos: Vector2i) -> Item:
	var it = Item.new()
	it.grid_pos = pos
	it.glyph = "?"
...
static func make_lightning_scroll(pos: Vector2i) -> Item:
	var it = Item.new()
	it.grid_pos = pos
	it.glyph = "?"
...
func _cast_lightning(user: Entity) -> String:
	...
	for e in GameState.entities:
		...
		var map_idx = GameState.map.idx(e.grid_pos.x, e.grid_pos.y)
		if GameState.map.visible[map_idx] and d < best_dist:
			best_dist = d
			nearest = e
	if nearest == null:
	...
	if not nearest.is_alive():
		msg += " It is slain!"
		GameState.remove_entity(nearest)
	return msg
