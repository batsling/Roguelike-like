...
func _enemy_text() -> String:
	...
	# stack below, because the second body is the one on that list the player did
	# not choose.
	var second: GoalEnemyData = GameLoop2.second_body()
	var second_line: String = "" if second == null else \
		"\n[color=#e06060]⚠ %s walked on beside it — its goal is on the stack below.[/color]" % second.display_name
	return "[b]Now playing:[/b] %s%s  ([i]%s / %s / dmg %d[/i])\n[b]GOAL (%s):[/b] %s%s" % [
		...
		GameLoop2.goal_text_for(GameLoop2.arrival()), second_line,
	]
